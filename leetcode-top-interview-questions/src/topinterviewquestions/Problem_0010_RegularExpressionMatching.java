...
public class Problem_0010_RegularExpressionMatching {
	...
	/**
	 * 1.要么pattern中的pi越界了，base情况
	 * 2.要么str中si越界了，base情况
	 * 3.pattern中的pi + 1字符不为*，然后看pattern中的pi + 1字符是否等于si的字符或者pi + 1字符为.
	 * //上述的情况就是考虑字符等位替换
	 * 4.考虑不能对的上的情况 ?* 必须变成0个
	 * 5.考虑能对的上的情况 要保证对上的字符个数是对齐的
	 * 
	 * @param s
	 * @param p
	 * @return
	...
	public static boolean process1(char[] str, char[] pattern, int si, int pi) {
		...
		// si 没越界 pi 没越界
		// pattern接下来没有*了，这时候不能把前一个字符变成0个
		// 要么当前字符相等，要么当前字符不能，pi字符为'.'，这样就可以变成和si一样的字符
		if (pi + 1 >= pattern.length || pattern[pi + 1] != '*') {
			return ((str[si] == pattern[pi]) || (pattern[pi] == '.')) && process1(str, pattern, si + 1, pi + 1);
		}
		// si 没越界 pi 没越界 pi+1位置为*
		if (pattern[pi] != '.' && str[si] != pattern[pi]) {
			return process1(str, pattern, si, pi + 2);
		}
		// 即便能配上但是也要考虑要变成0个进行回退
